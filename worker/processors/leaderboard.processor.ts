...
export const leaderboardUpdate: Processor<LeaderboardUpdateJob> = async (
  ...
  const session = await prisma.groupSession.findUnique({
    ...
    include: {
      ...
      members: { include: { user: { select: { id: true, name: true, username: true, avatarUrl: true, handicap: true } } } },
      rounds: {
  ...
  const entries: LeaderboardEntry[] = session.rounds.map((round) => {
    ...
    return {
      ...
      holesPlayed,
      currentHole: holesPlayed + 1 <= session.course.numHoles
        ? holesPlayed + 1
  ...
  try {
    emitLeaderboardUpdate(sessionId, entries);
    job.log(`Leaderboard broadcast: ${entries.length} players`);
  } catch {
    job.log("Socket.io emit failed — leaderboard computed but not broadcast");
  }
